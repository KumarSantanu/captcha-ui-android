...
class CaptchaLayout : RelativeLayout {
    ...
    private fun init() {
        val v: View = mInflater!!.inflate(R.layout.captcha_bottomsheet, this, true)
        ...
        val captchaHeaderView = v.findViewById<View>(R.id.captcha_header) as AppCompatTextView
        captchaHeaderView.text = captchaHeader

        val captchaDescriptionView = v.findViewById<AppCompatTextView>(R.id.captcha_description)
        captchaDescriptionView.text = captchaDescription

        val captchaInputView = v.findViewById<AppCompatEditText>(R.id.captcha_input)

        val captchaPositiveButton = v.findViewById<AppCompatButton>(R.id.captcha_positive_button)
        ...
        val captchaNegativeButton = v.findViewById<AppCompatButton>(R.id.captcha_negative_button)
    ...
    private fun setupAttributes(attrs: AttributeSet?) {
        ...
        captchaHeader =
            a.getString(R.styleable.CaptchaLayout_captcha_header) ?: "Captcha Verification"
        captchaDescription = a.getString(R.styleable.CaptchaLayout_captcha_description)
            ?: "The operation you are about to do cannot be reversed or undone. Please proceed if you are certain this is what you would like to do."
        verificationCodeLength = a.getInt(
            R.styleable.CaptchaLayout_captcha_code_length,
            CaptchaView.DEFAULT_VERIFICATION_CODE_LENGTH
        )

        // positive button
        positiveButtonTextColor =
            a.getColor(R.styleable.CaptchaLayout_captcha_positive_button_text_color, Color.BLACK)
        if (a.getString(R.styleable.CaptchaLayout_captcha_positive_button_text) != null) {
            positiveButtonText =
                a.getString(R.styleable.CaptchaLayout_captcha_positive_button_text)!!
        }

        // negative button
        negativeButtonTextColor =
            a.getColor(R.styleable.CaptchaLayout_captcha_negative_button_text_color, Color.BLACK)
        if (a.getString(R.styleable.CaptchaLayout_captcha_negative_button_text) != null) {
            negativeButtonText =
                a.getString(R.styleable.CaptchaLayout_captcha_negative_button_text)!!
        }
