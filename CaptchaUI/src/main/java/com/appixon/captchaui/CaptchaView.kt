...
class CaptchaView @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (c in verificationCode.toCharArray()) {
            ...
            canvas.drawText(
                c.toString(), x.toFloat(), height / 2.toFloat(),
                mTextPaint
            )
            canvas.restore()
            x += fontWidth.toInt()
        ...
        for (index in 0..DEFAULT_OBFUSCATION_LINE_COUNT) {
            canvas.drawLine(
                obfuscationLinesList[lineIndex].startingXCoordinate,
                obfuscationLinesList[lineIndex].startingYCoordinate,
                obfuscationLinesList[lineIndex].endingXCoordinate,
                obfuscationLinesList[lineIndex].endingYCoordinate,
                mLinePaint
            )
            lineIndex += 1
        }
    ...
    private fun generateRandomLine(): LineAttributes {
        ...
        return LineAttributes(
            startingXCoordinate.toFloat(),
            startingYCoordinate.toFloat(),
            endingXCoordinate.toFloat(),
            endingYCoordinate.toFloat()
        )
    }
    ...
    private fun setupVerificationCodesRotationAngles(verificationCodeLength: Int) {
        for (verificationCodeIndex in (0..verificationCodeLength)) {
            rotationalAngleList.add(
                generateRandomRotationAngle(
                    MINIMUM_ROTATIONAL_ANGLE,
                    MAXIMUM_ROTATIONAL_ANGLE
                )
            )
        }
    }
